...
export class EmployeeAppointmentService {
  ...
  getApointmentDetails(employeeId:string, id:number):Observable<EmployeeAppointment>{
    console.log('AppoinId = '+id)
    return this._http.get<EmployeeAppointment>(`${this.appointmentAPI}/EmployeeAppointmentDetails?EmployeePID=${employeeId}&appointmentId=${id}`);
  }
